...
<aside class="w-64 bg-slate-950/80 border-r border-slate-800/40 backdrop-blur-lg flex flex-col justify-between h-screen p-6 select-none shrink-0 z-20">
    <div class="space-y-8">
        ...
        <nav class="space-y-1.5">
            ...
            <a href="/dashboard" wire:navigate class="flex items-center space-x-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 <?php echo e(request()->is('dashboard') ? 'bg-brand-600/10 border border-brand-500/20 text-brand-400 shadow-sm' : 'text-slate-400 hover:bg-slate-900/60 hover:text-slate-200 border border-transparent'); ?>">
            ...
            <a href="/pohon-kinerja" wire:navigate class="flex items-center space-x-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 <?php echo e(request()->is('pohon-kinerja') ? 'bg-brand-600/10 border border-brand-500/20 text-brand-400 shadow-sm' : 'text-slate-400 hover:bg-slate-900/60 hover:text-slate-200 border border-transparent'); ?>">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 11H5m14 0a2 2 0 012 2v6a2 2 0 01-2 2H5a2 2 0 01-2-2v-6a2 2 0 012-2m14 0V9a2 2 0 00-2-2M5 11V9a2 2 0 012-2m0 0V5a2 2 0 012-2h6a2 2 0 012 2v2M7 7h10"></path></svg>
                <span>Pohon Kinerja</span>
            </a>
            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>

            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->user()->isOpd() || auth()->user()->isAdmin()): ?>
            <a href="/assessment-grid" wire:navigate class="flex items-center space-x-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 <?php echo e(request()->is('assessment-grid') ? 'bg-brand-600/10 border border-brand-500/20 text-brand-400 shadow-sm' : 'text-slate-400 hover:bg-slate-900/60 hover:text-slate-200 border border-transparent'); ?>">
                <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2m-3 7h3m-3 4h3m-6-4h.01M9 16h.01"></path></svg>
                <span>Lembar Kerja Evaluasi</span>
            </a>
            <?php endif; ?><?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if ENDBLOCK]><![endif]--><?php endif; ?>

            <?php if(\Livewire\Mechanisms\ExtendBlade\ExtendBlade::isRenderingLivewireComponent()): ?><!--[if BLOCK]><![endif]--><?php endif; ?><?php if(auth()->user()->isBappeda() || auth()->user()->isInspektorat() || auth()->user()->isAdmin()): ?>
            <a href="/score-adjustment" wire:navigate class="flex items-center space-x-3 px-3 py-2.5 rounded-xl text-sm font-medium transition-all duration-300 <?php echo e(request()->is('score-adjustment') ? 'bg-brand-600/10 border border-brand-500/20 text-brand-400 shadow-sm' : 'text-slate-400 hover:bg-slate-900/60 hover:text-slate-200 border border-transparent'); ?>">
...
</aside><?php /**PATH D:\PROGRAM CODING\spip\spip\storage\framework\views/livewire/views/783e5dbb.blade.php ENDPATH**/ ?>