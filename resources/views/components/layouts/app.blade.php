...
<head>
    ...
    <title>{{ $title ?? 'E-SPIP Terintegrasi' }}</title>
            ...
            <header class="h-16 flex items-center justify-between px-8 border-b border-slate-800/40 bg-slate-950/40 backdrop-blur-md z-10">
                ...
                <div class="flex items-center space-x-4">
                    <span class="text-xs text-slate-400 bg-slate-800/60 px-3 py-1.5 rounded-full border border-slate-700/40">
                        Fis. Year: {{ session('fiscal_year', date('Y')) }}
                    </span>
                    <div class="flex items-center space-x-2">
                        ...
                        <div class="hidden md:flex flex-col text-left">
                            <span class="text-xs font-semibold text-slate-200">{{ auth()->user()->name ?? 'Guest' }}</span>
                            <span class="text-[10px] text-slate-400 uppercase tracking-wider">{{ auth()->user()->role ?? 'Public' }}</span>
                        </div>
                    </div>
                </div>
            </header>
